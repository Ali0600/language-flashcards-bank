...
- **EAS Update channels and branches are not the same thing.** `eas update --branch X` publishes to a branch. Builds query a channel. For updates to flow, a channel must exist that points to the branch. After the first build for a new profile, run `eas channel:create <profile-name>` if `eas channel:list` is empty. The build profile name (`production`) is the channel name baked into the binary; the channel must match.
