...
- `app/scan-category/[id].tsx` — Second step of the capture wizard. 2-column grid of all 11 category tiles + Uncategorized matching the Library Folders style. Currently-selected tile gets a 2.5px border + filled tint icon background + checkmark badge; the tile Gemini originally picked carries a "Gemini" badge so the user can see what they're changing. "Save & Finish" calls `updatePhotoCategory` (no-op when unchanged) then `router.dismissTo('/(tabs)')`. Reached via `router.replace` from scan results, so back-navigation skips the (now-stale-after-removals) scan list.
...
**Route registration:** new screens must be added to the `<Stack>` in `app/_layout.tsx`. Expo Router's typed routes don't pick up new files until `npx expo start` regenerates `.expo/types` — until then, cast as `'/route' as never` when calling `router.push`. Existing examples: `/settings`, `/folder/[slug]`, `/ignored`, `/focus`, `/scan-category/[id]`.
