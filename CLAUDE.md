...
- `settings` — key/value/updatedAt, JSON-serialized values. Keys: `dailyNewCardLimit`, `playInSilentMode`, `autoCreateReverseCards`, `autoPlayWord`.
...
- `app/settings.tsx` — Daily new-card limit stepper, silent-mode toggle, **auto-play word on reveal toggle** (mirrors the speaker icon in the Study header), auto-create-reverse-cards toggle, bulk-backfill-reverses button, link to ignored-words screen, and a **Danger zone** at the bottom with a red-outlined "Delete all cards" button (two-step confirmation showing the actual count, transactional delete of review logs + sightings + cards via `deleteAllCards`).
...
- `use-settings.ts` — `useDailyNewCardLimit`, `usePlayInSilentMode`, `useAutoCreateReverseCards`, `useAutoPlayWord`. Boolean settings share `useBooleanSetting` internally — when adding another boolean setting, use that helper rather than copy-pasting the focus-effect dance.
...
**Shared components:**
- `components/study-session.tsx` — flippable card UI + FSRS rating buttons + optional "Seen often" rail. Used by both the global Study tab (`app/(tabs)/study.tsx`) and per-folder Study (`app/study-folder/[slug].tsx`). The hook supplying the queue is the only meaningful difference between them. The top header has a quick-toggle speaker icon mirroring the `autoPlayWord` setting (tap to flip; the icon swaps `speaker.wave.2.fill` ↔ `speaker.slash.fill`). When the back is revealed, the lemma auto-plays through `speakGerman` (gated by the setting, read via ref so a mid-card toggle doesn't interrupt in-flight audio) and a small inline speaker next to the lemma pulses a tint-colored halo while audio is playing (`Animated.loop` of opacity+scale on a 550ms cycle, `useNativeDriver: true`). The bottom Listen button now plays only the example sentence and is hidden when the card has none.
