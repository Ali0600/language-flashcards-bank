...
- `app/folder/[slug].tsx` — Three modes in one screen. (1) Parent without sub-cats → flat card list (current behavior). (2) Parent in `FOLDERS_WITH_SUBCATEGORIES` and no `sub` query → grid of sub-category tiles plus an "Uncategorized within this parent" tile when any photos have a null `sub_category_id`. An "All apps" tile prepends when there's more than one bucket (deduplicated card count). (3) Same parent with `?sub=<id>` (Uncategorized bucket = `?sub=null`, parent-wide = `?sub=all`) → flat card list filtered accordingly. The card-list mode renders a small **Re-analyze cards** outline button under the header (confirmation Alert → `/audit/[slug]?sub=...`) and a floating **Study** button at the bottom that pushes to `/study-folder/[slug]?sub=...` with the same filter.
- `app/audit/[slug].tsx` — Folder audit screen. Fetches all forward cards in scope (mirrors `?sub=` semantics from the folder route), batches them 10-at-a-time through Gemini via `services/audit.ts`, and shows two tabs: **Corrections** (per-issue checkboxes for field fixes — lemma / gender / translation / examples / plural; pre-checked by default; Apply commits via a single transactional `applyAuditChanges`) and **Review** (cards Gemini flagged as not everyday-vocab, with rationale; tap a row to open `/card/[id]` and delete or keep manually). Aborts the in-flight Gemini calls via `AbortController` when the user navigates away mid-audit.
...
- `audit.ts` — Folder-scoped flashcard re-analysis. `fetchFolderCardsForAudit(parentSlug, subId)` returns every forward card in scope (mirrors `useFolderDueCards`'s photo predicate, drops the due-date filter). `auditCards(cards, signal, onProgress)` sends them to Gemini in batches of 10 with a SYSTEM_PROMPT that asks for (a) per-field correction issues — lemma / gender / translation / exampleDe / exampleEn / plural — and (b) a separate "everydayRationale" string flagging cards outside everyday-vocab. Retries each batch once on network errors; aborts cleanly via the supplied signal. `applyAuditChanges(changes)` groups changes by card and updates all in one `db.transaction`; gender "" / plural "" map to `null`, and `lemma` changes that would violate the `(lemma, direction)` unique constraint throw out to the caller (the UI alerts).
